...
int main()
{
	...
	//Объект синхронизации для защиты общих ресурсов от одновременного доступа
	...
	for (int i = 0; i < num; i++)
	{
		...
		while (true)
		{
			...
			if (numToStop >= 0 && numToStop < num && isWorking[numToStop])
			{
				...
				ResetEvent(stopEvents[numToStop]);//Объекты синхронизации, которые могут находиться в сигнальном или несигнальном состоянии
